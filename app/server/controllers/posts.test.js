...
import { getPostsFromNeo4j } from './neo4j.js'
...
describe('createPost', function () {
    ...
    it('should successfully create a post and return status 201', async function () {
        ...
        expect(res.json.calledOnce).to.be.true
        expect(res.json.args[0][0]).to.have.property('newPost')
        expect(res.json.args[0][0].newPost).to.deep.equal(req.body)
    })
    ...
    it('should update user hashTags and save the user', async function () {
        ...
        // Verify findByIdStub was called with the createdBy ID
        expect(findByIdStub.calledWith(req.body.createdBy)).to.be.true

        // Check if user hashTags were updated and user was saved
        const userArg = findByIdStub.returnValues[0]
        await userArg.save()
        expect(userArg.save.calledOnce).to.be.true
        expect(userArg.hashTags).to.include.members(hashTags)
    })
})
...
describe('getRecommendedTweets', function () {
    ...
    it('should handle error if fetching posts fails', async function () {
        getPostsStub.rejects(new Error('Failed to fetch posts'))

        await getRecommendedTweets(req, res)

        expect(res.status.calledWith(404)).to.be.true
        expect(res.json.calledOnce).to.be.true
        expect(res.json.args[0][0]).to.have.property('message', 'Failed to fetch posts')
    })
})
